...
2. 各トラック: 保存済み `timelineSlots` の musical 紐付けを維持
...
| 手動ダンプ | `musicalSlotDiagDumpOriginBindings(0)` — SwapUnit 列と musical 紐付けのサマリー |
| E キー | `swap/before` → `swap/apply` → `swap/after-cache` → `swap/done` |
...
| `origin/cache-merge` | — | cache マージ時の identity 不一致診断 |
| `session/restore/*` | `session/restore/phrase`, `session/restore/origin`, `session/restore/done` | 復元直後の全 Ex トラック musical 紐付けレポート（`=== 読み取り用サマリー ===` 行） |
